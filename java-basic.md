...
⭐️ -> 왜 힙만 저장 ?
그 이유는 여러가지인데 가비지 컬렉터가 힙 영역에서만 활동을 하는데
힙 영역은 동적 메모리 할당이 가능한데 객체의 크기를 저장하는데에 있어 자유롭고
이로 인한 수명주기 관리에 적합하고 멀티스레딩을 지원해 여러 스레드가 동시에 접근할 수 있고
자동 정리가 되어 힙에 저장하는게 유리합니다.
...
추상 클래스는 기본적으로 상속과 확장하여 사용하기 위한 것이며 인터페이스는 동일한 사용방법과 동작을 보장하기 위한 것으로
차이를 나타내고있습니다.

추상 클래스는 공통적인 속성과 기능을 가진 관련 클래스들의 기본 틀을 제공하는 데 적합합니다. 이를 통해 코드 재사용성을 높이고 관련 클래스들 간의 일관성을 유지할 수 있습니다.
인터페이스는 클래스의 특정 행동을 정의하는 데 중점을 둡니다. 서로 다른 클래스들이 동일한 방식으로 동작해야 할 때 사용하며, 다중 구현을 통해 유연한 설계가 가능합니다.
두 가지 모두 코드 재사용과 다형성을 증진시키지만, 추상 클래스는 '무엇인가'를 나타내는 반면, 인터페이스는 '무엇을 할 수 있는가'를 나타냅니다
